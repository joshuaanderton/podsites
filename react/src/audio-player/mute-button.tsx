import { type PlayerAPI } from '@/audio-player/hooks/use-audio-player'
...
export function MuteButton({ player }: { player: PlayerAPI }) {
  return (
    <button
      type="button"
      className={clsx(
        'rounded-md focus:outline-none hover:text-neutral-700 transition-colors',
        player.muted ? 'text-neutral-700' : 'text-neutral-400'
      )}
      onClick={() => player.toggleMute()}
      aria-label={player.muted ? 'Unmute' : 'Mute'}
    >
      <div className="absolute -inset-4 md:hidden" />
      <MuteIcon muted={player.muted} className="size-5 fill-current stroke-current" />
    </button>
  )
}
